...
import tools.tm.model_printing as mp
...
def main():
    ...
    a.add_argument(
        ...
        required=True,
        type=str,
        help="what you would like the .pdf files to start with (e.g. baseline_model_ will produce a visualization for topic 1 called baseline_model_1.pdf",
    )
    a.add_argument(
        "-seeds",
        dest="seeds",
        required=True,
        type=str,
        help="which all_seeds.txt file do you want to read the seed terms from? (base: <project_root>/intput/seeds/)",
    )
    a.add_argument(
        "-k",
        dest="k",
        default=30,
        type=int,
        help="how many terms (maximum) should appear in the visualization?",
    )
    a.add_argument(
        "-num_topics",
        dest="num_topics",
        type=int,
        help="create visuals for the first num_topics topics in a model. If NULL, visualizes every topic.",
    )
    a.add_argument(
        "-title",
        dest="title_base",
        default="",
        help="title of each visualization will be '<title> topic X'",
    )
    a.add_argument(
        "-show_prob",
        dest="show_prob",
        default=True,
        type=bool,
        help="set to true to display exact probability values (False: no values diplayed)",
    )
    a.add_argument(
        "-colors",
        dest="p_tw",
        default=True,
        type=bool,
        help="set to true to color-code by p(t|w) 'exclusivity' score (False: all bars will be white)",
    )
    a.add_argument(
        "-logscale",
        dest="logscale",
        default=True,
        type=bool,
        help="set to true to display x-axis on a log scale (False: linear scale)",
    )
    a.add_argument(
        "-show_seeds",
        dest="show_seeds",
        default=False,
        type=bool,
        help="set to True to display seeds (False (default): hide all seed terms)",
    )
    ...
    if opts.num_topics:
        ...
        for i in range(0, opts.num_topics):
            ...
            mp.visualize_topic(
                ...
                show_prob=opts.show_prob,
                chart_name=name,
                p_tw=opts.p_tw,
                output_file=OUT,
                logscale=opts.logscale,
                show_seeds=opts.show_seeds,
            )
