...
def load_dictionary():
    return gensim.corpora.Dictionary.load(
        BASE_DIRECTORY + "models/bow/" + DICTIONARY_NAME + ".gensim"
    )
...
def load_corpus():
    ...
    return pickle.load(
        open(BASE_DIRECTORY + "models/bow/" + CORPUS_NAME + ".pkl", "rb")
    )
...
def main():
    ...
    (w, d) = (NP.array(wordvec, dtype=int), NP.array(docvec, dtype=int))
    ...
    print(
        "terms: {}".format(
            [[dict.get(idx) for idx in wordlist] for wordlist in wordlists]
        )
    )
    ...
    if config["FILE_PATHS"]["log_file"] is not None:
        mp.log_counts(
            config["FILE_PATHS"]["log_file"], counts, len(estphi), len(esttheta)
        )
