...
import math
import tools.tm.model_printing as mp  # for convenience, get seedless topic lists at the same time
...
def main():
    ...
    a.add_argument(
        ...
        required=True,
        type=str,
        help="the topic model to use (_phi)",
    )
    a.add_argument(
        "-topics",
        dest="num_topics",
        required=True,
        type=int,
        help="number of topics to be calculated",
    )
    a.add_argument(
        "-k",
        dest="k",
        required=True,
        type=int,
        help="number of terms in a topic to include",
    )
    a.add_argument(
        "-outdir",
        dest="output_dir",
    ...
    a.add_argument(
        ...
        required=True,
        type=str,
        help="what to call the output file",
    )
    a.add_argument(
        "-csv",
        dest="use_csv",
        ...
        default=False,
        type=bool,
        help="use this flag to get a csv instead of tab-separated txt file",
    )
    a.add_argument(
        "-seedless",
        dest="print_seedless",
        required=False,
        type=bool,
        help="use this flag to hide seed terms from results",
    )
    a.add_argument(
        "-seeds",
        dest="seeds",
        required=False,
        type=str,
        help="REQUIRED IF SEEDLESS: file containing all seed terms",
    )
    ...
    for topic in range(opts.num_topics):
        ...
        outfile_name = (
            opts.output_dir + "p_topic_given_word_" + opts.file_base + str(topic)
        )
        ...
        if opts.print_seedless:  # only print terms not in seed list
            ...
            if opts.use_csv:
                print_to_csv_seedless(
                    outfile_name, dict, opts.k, k_best, t_probabilities, seeds
                )
            else:
                print_to_txt_seedless(
                    outfile_name, dict, opts.k, k_best, t_probabilities, seeds
                )
