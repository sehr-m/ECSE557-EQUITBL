...
def main():
    ...
    a.add_argument(
        ...
        required=True,
        type=str,
        help="path to .ini configuration file (for specifying file paths)",
    )
    opts = a.parse_args()
    config = ConfigParser()
    ...
    IGNORE = (
        config["PREPROCESSING"]["ignore_pos"].replace(" ", "").strip("[]").split(",")
    )
    STOPWORDS = (
        config["PREPROCESSING"]["stopwords"].replace(" ", "").strip("[]").split(",")
    )
    ...
    lemma_dictionary = preproc.get_chunked_pos_lemmas_dictionary(
        df, IGNORE, STOPWORDS, CHUNK_SIZE
    )
